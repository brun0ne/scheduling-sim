...
export default class AnimationGUI implements IAnimationGUI {
    ...
    init(): void {
        ...
        this.see_code_button.addEventListener("click", () => {
            if (this.menu.memory.algorithm == null)
                return;

            const algorithmName = this.menu.memory.algorithm.name;
    ...
    drawCalls(START_X?: number, START_Y?: number, WIDTH?: number, HEIGHT?: number, callsToDraw?: any): void {
        ...
        const newCall = this.menu.memory.previous_page_calls.at(-1);
        if (newCall == null)
            return;
    ...
    refreshInfo(): void {
        ...
        if (memory.algorithm == null)
            throw new Error("call startAnimation() first!");

        this.info_algorithm!.innerHTML = memory.algorithm.name;
        ...
        this.info_total_page_hits!.innerHTML = (memory.previous_page_calls.length -  memory.total_page_faults).toString();
        this.info_total_page_faults!.innerHTML = memory.total_page_faults.toString();

        this.info_in_queue!.innerHTML = memory.page_call_queue.length.toString();
        this.info_finished!.innerHTML = memory.previous_page_calls.length.toString();
    }
